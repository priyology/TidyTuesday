...
#### Question: What % of each diversity 'category' is enrolled in each state? ====
...
diversity_school$enrollment <- as.integer(diversity_school$enrollment)
...
CategoryEnrollment.Plot <- ggplot(category.enrollment.eth, aes(x="", y=enrollment.percent.eth, fill=category)) +
  geom_bar(stat="identity", width=1, color="white") +
  facet_wrap(. ~ state) +
  coord_polar("y", start=0) +
  labs(title="Demographics of College Student Enrollment By State",fill = "Student Demographics") +
  theme_void() # remove background, grid, numeric labels

CategoryEnrollment.Plot

png('fig_output/CollegeStudentEnrollmentByState_11Mar2020.png',width=1000,height=1000,units="px",bg = "transparent")

print(CategoryEnrollment.Plot)
